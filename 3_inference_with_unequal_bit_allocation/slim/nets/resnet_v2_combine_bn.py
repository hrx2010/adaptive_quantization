...
def bottleneck(inputs, depth, depth_bottleneck, stride, rate=1,
  ...
  with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
    ...
    def f1(): 
      temp = slim.conv2d(preact, depth_bottleneck, [1, 1], stride=1,
                           scope='conv1')
      return temp

    def f2():
      temp = slim.conv2d(preact, depth_bottleneck, [1, 1], stride=1,
                           normalizer_fn=None, activation_fn=tf.nn.relu, scope='conv1/pie')
      
      codebook_conv_1 = tf.get_variable(name='conv1/pie/codebook', shape=[256,])
    ...
    def h1():
      ...
      temp = resnet_utils.conv2d_same(residual, depth_bottleneck, 3, stride,
                                        rate=rate, scope='conv2')
      return temp 

    def h2()  :
      if stride == 1:
        temp = slim.conv2d(residual, depth_bottleneck, 3, stride=1, rate=rate,
                       padding='SAME', normalizer_fn=None, activation_fn=tf.nn.relu, scope='conv2/pie')

        codebook_conv_2 = tf.get_variable(name='conv2/pie/codebook', shape=[256,])
        ...
        inputs = tf.pad(residual,
                        [[0, 0], [pad_beg, pad_end], [pad_beg, pad_end], [0, 0]])
        temp = slim.conv2d(inputs, depth_bottleneck, 3, stride=stride,
                           rate=rate, padding='VALID', normalizer_fn=None, activation_fn=tf.nn.relu, scope='conv2/pie')
...
def resnet_v2_50_combine_bn(inputs,
                 num_classes=None,
                 is_training=True,
                 global_pool=True,
                 output_stride=None,
...
resnet_v2_50_combine_bn.default_image_size = resnet_v2.default_image_size
